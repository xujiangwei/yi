define(function(require, exports, module) {
	...
	var ChooseTime=BaseClass.extend({	
		...
		Modle : {
			modle : ['<table border="0" cellspacing="0" cellpadding="0">',
                       '<tr>',
                          ...
                          '<button   type="button"  class="btn btn-primary"  style="margin-left:5px;" id="cancel@{id}">取消</button>',
                        '</div>',
                          '</td>',                          
                       '</tr>',
                      '</table></form>'
		...
		getRange:function(){
			return $$("startmonth"+this.id).value+" - "+$$("endmonth"+this.id).value;
		},
		getStartTime:function(){
			return $$("startmonth"+this.id).value;
		},
		getEndTime:function(){
			return $$("endmonth"+this.id).value;
		},
        CreateElem:function(){
