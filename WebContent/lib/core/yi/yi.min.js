...
undefined){console.log('[Yi#Mod] Can not find "data-mod" attribute value.');return}var version=target.data("modVer");if(version===undefined){console.log('[Yi#Mod] Can not find "data-mod-ver" attribute value.');return}var context=this.context;var url=this.context+"modloader"+"/"+modName+"/"+version;$.post(url,function(data,textStatus,jqXHR){if(args!==undefined)data["args"]=args;data["context"]=context;target.fetch(data)},"json").fail(function(){console.log('[Yi#Mod] Failed requests "'+url+'".')})};
ModManager.prototype.debug=function(containerId,mod){var container=$("#"+containerId);mod["context"]=this.context;container.fetch(mod)};global.yi=new Yi})(this);
