 ...
 *        lastActiveIndex)
 * 
 * @description 还可以补充通过参数加载卡片页面以及手动添加卡片页面 updated on 2014-03-11
 * 
 */
define(function(require, exports, module) {
	'require:nomunge,exports:nomunge,module:nomunge';

	require('./card-layout.css');
	...
	(function() {
		var CardLayout = extend(Base, {
			...
			afterRender : function(parent) {
				...
				this.cards = this.el.children('.' + this.baseCls + '-card');
				if (this.cards.size() > 0) {
					var len = this.cards.size(), i;
					for (i = 0; i < len; i++) {
						var card = this.cards.eq(i);
						if (card.hasClass(this.baseCls + '-card-active')) {
			...
			setActive : function(index) {
				...
				this.cards.eq(index).addClass(this.baseCls + '-card-active');
				this.trigger('activate', this, index, this.lastActiveIndex);
