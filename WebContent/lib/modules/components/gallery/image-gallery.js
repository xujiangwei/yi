 ...
 * 
 * @description updated on 2014-02-13
 * 
 */
	...
	(function() {
		var ImageGallery = extend(Gallery, {
			...
			onItemRender : function(item, container, id, data, isAdd) {
				...
				item.el.addClass(this.baseCls + '-image-item').html(
						'<div class="'
								+ this.baseCls
								+ '-item-top"><img '
								...
								+ (this.imageHeight ? 'height="'
										+ this.imageHeight + 'px"' : '')
								+ ' src="' + src + '" /></div><div class="'
								+ this.baseCls + '-item-bottom" title="'
								+ title + '">' + title + '</div>');
			},
			/**
			 * 修改标题
			 ...
			 * @argument
			 * 
			 * 1、item String/Number/Object 参数可以是item对象、唯一标识或者UI中的位置（从0开始）
			 * 
			 * 2、title String: 新的标题
			 */
			setTitle : function(item, title) {
				...
				if (item) {
					...
					item.el.children('.' + this.baseCls + '-item-bottom').html(
