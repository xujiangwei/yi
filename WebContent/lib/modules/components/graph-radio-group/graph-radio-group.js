 ...
 *        Event e)
 * 
 * @description updated on 2014-03-12
 * 
 */
define(function(require, exports, module) {
	...
	function onItemClick(e) {
		var comp = Base.get(e.data.compId), baseCls = comp.baseCls, $item = $(e.target)
				.closest('.' + baseCls + '-item');
		if ($item.size() > 0) {
	...
	(function() {
		...
		var GraphRadioGroup = extend(Base, {
			...
			afterRender : function(container) {
				...
				for (i = 0; i < len; i++) {
					var d = data[i], url = d && d.url, $item = $(this.items[i])
							.addClass(baseCls + '-item').data('item', d), index = $item
							.index();
					if (index > 0) {
						$item.css({
					...
					this["addTo" + utils.firstLetterToUpperCase(this.region)]($item);
			...
			renderItem : function() {
				return '<div class="' + this.baseCls + '-graph-' + this.region
						+ ' ' + this.baseCls + '-graph"></div>';
			},
			addToSouth : function(item) {
				item.append(this.renderItem()).addClass(this.baseCls
						+ "-item-south");
			},
			addToNorth : function(item) {
				item.prepend(this.renderItem()).addClass(this.baseCls
						+ "-item-north");
			},
			...
			clickItem : function(item, e) {
				...
				$('.' + baseCls + '-graph', this.el).removeClass(baseCls
						+ '-selected');
				$('.' + baseCls + '-graph', item).addClass(baseCls
						+ '-selected');
				$('input', item).prop('checked', true);
				this.trigger('itemclick', this, item, item.data('item'), e);
			},
			/**
			 * 获取选中项,返回选中的数据
			 */
			getSelectedItem : function() {
				return $('.' + this.baseCls + '-selected', this.el).parent()
						.data('item');
			},
